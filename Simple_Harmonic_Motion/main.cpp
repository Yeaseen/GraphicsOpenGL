...
void firstBall(double ampl){

    double xaxis = rope * sin(ampl * (3.1416 /180));
    double yaxis = rope * cos(ampl * (3.1416 /180));
    yaxis = rope - yaxis + high;
...
void secondBall(double ampl){
    ...
    glColor3f(1, 0, 0);
		glBegin(GL_LINES);{
			glVertex3f( 0,rope,15);
			glVertex3f(-xaxis,yaxis,15);
		}glEnd();
    glColor3f(1, 0, 0);
    glTranslatef(-xaxis,yaxis,15);
    drawSphere(8,24,80);
    glTranslatef(xaxis,-yaxis,-15);
...
void display(){
	...
	****************************/
    ...
    firstBall(amplitude1);

    secondBall(amplitude2);
